...
class Maze(tk.Tk, object):
    ...
    def _build_maze(self):
        ...
        self.target1 = self.canvas.create_rectangle(
            3*UNIT,7*UNIT,4*UNIT,8*UNIT,
            fill='light salmon')
        self.target2 = self.canvas.create_rectangle(
            5*UNIT,9*UNIT,6*UNIT,10*UNIT,
            fill='tomato')
    ...
    def move_robot_and_time(self, robot, start, goal, callback=None):
        ...
        if step < len(path):
            ...
            for other_robot in robots:
                if other_robot != robot and self.canvas.coords(robot) == self.canvas.coords(other_robot):
                    # Collision detected, change color to hot pink
                    self.canvas.itemconfig(robot, fill='hot pink')
                    self.canvas.itemconfig(other_robot, fill='hot pink')
            ...
            # Robot reached the goal position
            if hasattr(self, 'start_time') and self.start_time is not None:
                end_time = time.time()
                total_time = end_time - self.start_time
                print(f"Total time taken: {total_time} seconds")
            else:
                print("Error: start_time not properly set")

            if callback:
                callback()


    def move_robot(self, robot, start, goal, start_time=None, callback=None):
        came_from, _ = self.a_star_search(start, goal)
        path = self.reconstruct_path(came_from, start, goal)
        self.move_robot_step(robot, path, callback=lambda: self._move_robot_callback(start_time, callback))

    def _move_robot_callback(self, start_time, callback):
    ...
    def start(self):
        self.start_time = time.time()  # Set start time explicitly
        # 로봇 1 이동
        self.move_robot_and_time(self.rect1, (origin1[0] // UNIT, origin1[1] // UNIT), (3, 7),
                                 callback=lambda: self.move_robot_and_time(self.rect1, (3, 7), (origin1[0] // UNIT, origin1[1] // UNIT)))
        # 로봇 2 이동
        self.move_robot_and_time(self.rect2, (origin2[0] // UNIT, origin2[1] // UNIT), (5, 9),
                                 callback=lambda: self.move_robot_and_time(self.rect2, (5, 9), (origin2[0] // UNIT, origin2[1] // UNIT)))
        # 로봇 3 이동
        self.move_robot_and_time(self.rect3, (origin3[0] // UNIT, origin3[1] // UNIT), (7, 7),
                                 callback=lambda: self.move_robot_and_time(self.rect3, (7, 7), (origin3[0] // UNIT, origin3[1] // UNIT)))
